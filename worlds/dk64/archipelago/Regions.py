...
def create_regions(multiworld: MultiWorld, player: int, spoiler: Spoiler, options=None):
    ...
    for region_id in all_logic_regions:
        ...
        if region_id in all_collectible_regions.keys():
            ...
            # Include coins when shops_dont_cost is enabled so they appear in the playthrough as prerequisites for shops
            # Coins are NOT tracked for banana medal logic (which uses bunches/bananas/balloons only)
...
def create_region(
    ...
    for collectible in collectibles:
        ...
        # Coins use a different naming format (no level) since they're not tied to specific levels like CBs
        if collectible.type == Collectibles.coin:
            # Coins are progression_skip_balancing - they're required for shops but don't drive item placement
