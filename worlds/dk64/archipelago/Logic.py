...
class LogicVarHolder:
    ...
    def __init__(self, spoiler, player):
        ...
        # Handle Helm level shuffling assumption
        # We can only assume level 8 entry if Helm is actually in position 8
        # If another level is in position 8, it needs normal key requirements to access upper Krem Isle
        if hasattr(settings, 'level_order') and 8 in settings.level_order:
            level_8_is_helm = settings.level_order[8] == Levels.HideoutHelm
        else:
            # If level_order isn't available or position 8 doesn't exist, assume vanilla (Helm is in position 8)
            level_8_is_helm = not getattr(settings, 'shuffle_helm_location', False)
        self.assumeLevel8Entry = level_8_is_helm
    ...
    def IsLevelEnterable(self, level):
        ...
        # Special handling for Helm when it's shuffled into the level order
        if level == Levels.HideoutHelm and self.assumeLevel8Entry:
            return True
